package com.company;

import java.util.Scanner;

public class ComputeTax {
    public static void main(String[] args){
        System.out.println("Welcome to Compute Tax!");
        System.out.println("Please enter your filing status");
        System.out.println("Enter 0 for single filers");
        System.out.println("Enter 1 for married filing jointly or qualified widow(er)");
        System.out.println("Enter 2 for married filing separately");
        System.out.println("Enter 3 for head of household");

        Scanner input = new Scanner(System.in);
        int status = input.nextInt();

        // Prompt the user to enter taxable income
        System.out.print( "Enter the taxable income: " );
        double income = input.nextDouble();

        // Compute tax
        double tax = 0 ;

        if (status == 0) {
            // Compute tax for single filers
            if (income <= 8350)
                tax = income * 0.10;
            else if (income < 33950) {
                tax = 8350 * 0.10 + (income - 8350 ) * 0.15;
            }
            else if (income <= 82250 ) {
                tax = 8350 * 0.10 + ( 33950 - 8350 ) * 0.15 + (income - 33950 ) * 0.25;
            }
            else if (income <= 171550 ) {
                tax = 8350 * 0.10 + ( 33950 - 8350 ) * 0.15 + ( 82250 - 33950 ) * 0.25 + (income - 82250 ) * 0.28 ;
            }
            else if (income <= 372950 ) {
                tax = 8350 * 0.10 + ( 33950 - 8350 ) * 0.15 + ( 82250 - 33950 ) * 0.25 + ( 171550 - 82250 ) * 0.28
                        + (income - 171550 ) * 0.33 ;
            }
            else
                tax = 8350 * 0.10 + ( 33950 - 8350 ) * 0.15 + ( 82250 - 33950 ) * 0.25 + ( 171550 - 82250 ) * 0.28
                        + ( 372950 - 171550 ) * 0.33 + (income - 372950 ) * 0.35 ;


        }
        else if (status == 1) {
            // Compute tax for married filing jointly or qualifying widow(er)
            // Left as an exercise

        }
        else if (status == 2) {
            // Compute tax for married filing separately
            // Left as an exercise

        }
        else if (status == 3) {
            // Compute tax for head of household
            // Left as an exercise

        }
        else {
            // Display wrong status
            System.out.println("Error: invalid status");

            //Invoking this method terminates the program
            //Status 0 indicates the program terminated normally
            //nonzero status indicates abnormal termination
            System.exit(1);

        }

        // Display the result
        System.out.println("Tax is " + (int)(tax * 100) / 100.0);

    }
}
